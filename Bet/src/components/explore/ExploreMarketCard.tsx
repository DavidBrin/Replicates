...
export function ExploreMarketCard({ view, now }: ExploreMarketCardProps) {
  ...
  return (
    <div className="relative flex flex-col rounded-(--radius-card) border border-(--border) bg-(--surface-1) p-4 transition-colors hover:border-(--border-2) hover:bg-(--surface-2)">
      <Link
      ...
      <div className="pointer-events-none relative z-10 flex flex-1 flex-col">
        {categoryLabel ? (
          <Badge tone="neutral" className="mb-2 w-fit tracking-wide uppercase">
            {categoryLabel}
          </Badge>
        ) : null}
        <p className="mb-3 line-clamp-2 text-[15px] font-semibold text-(--text-1)">{market.question}</p>
