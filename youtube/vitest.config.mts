 ...
 * repositories, the database adapters and the blob store all import it, so
 * every one of their suites would fail at module load. Aliasing it to an empty
 * module keeps the guard where it matters and out of the way where it does not.
 *
...
export default defineConfig({
  ...
  test: {
    ...
    css: false,
    /**
     * PGlite boots a WASM Postgres per suite and the muxer suites encode real
     * frames; the default 5s is not enough on a cold run. A flaky timeout here
     * would read as a schema bug or a codec bug rather than as a slow machine.
     */
    testTimeout: 30_000,
    hookTimeout: 30_000,
  },
