...
export async function scanVideo(
  ...
  /**
   * One placeholder per id, not one comma-joined string split back apart.
   *
   * This read `string_to_array($1, ',')` over `ids.join(",")`, which makes the
   * comma a delimiter in a value it has no business delimiting. A work id
   * containing a comma became two ids, neither of which exists, so the work's
   * policy was not found and — because the loop below skips a candidate with
   * no policy — the match raised **no claim at all**. A rights-holder whose id
   * happened to contain the separator would have had Content ID silently
   * decline to protect them, with nothing in the logs to say so.
   *
   * Ids here are generated by `nanoid` and do not contain commas today. That
   * is a property of the generator, not of the column, and the query should
   * not depend on it: `reference_works.id` is `text`, and the seed and the
   * demo importer both accept ids from outside.
   */
  const placeholders = ids.map((_, index) => `$${index + 1}`).join(", ");
  const works = await db.query<{ id: string; policy: ContentPolicy }>(
    `select id, policy from reference_works where id in (${placeholders})`,
    ids,
  );
