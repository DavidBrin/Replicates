 ...
 */
function drawCannon(b: Brush, p: PropDef): void {
  const ctx = b.ctx;
...
function drawPauldron(b: Brush, p: PropDef): void {
  ...
  // shoulder rather than sitting on it. Kept to roughly a head's width: the
  // first pass at this was twice the size and turned her chest into one red
  // slab, which reads as a bib and not as armour.
  //
  ...
  ellipse(ctx, 0.06, -0.2, 0.98, 0.78, -0.2);
  b.fill(p.colour);
  if (b.mode === "body" && p.detail) {
    ...
    ellipse(ctx, 0.1, 0.3, 0.9, 0.34, -0.2);
    b.fill(p.detail);
  }
}
...
export const rig: CharacterRig = {
  ...
  props: [
    ...
    { kind: "custom", bone: "upperArmL", at: 0.04, size: 1.62, across: 0.05, colour: "secondary", detail: GUNMETAL, flip: true, draw: drawPauldron },
    { kind: "custom", bone: "upperArmR", at: 0.04, size: 1.78, across: 0.08, colour: "secondary", detail: GUNMETAL, draw: drawPauldron },
